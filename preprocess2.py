...
def fill_empty_data(df: DataFrame) -> DataFrame:
    ...
    # Convert empty missing values
    df["Build Year"] = df.groupby(["Locality", "Type"])["Build Year"].transform(
        lambda x: x.fillna(x.median())
    )
    df["Facades"] = df.groupby(["Locality", "Type"])["Facades"].transform(
        lambda x: x.fillna(x.median())
    )
    df["Kitchen Type"] = df.groupby(["Locality", "Type"])["Kitchen Type"].transform(
        lambda x: x.fillna(x.median())
    )
    df["Terrace Surface"] = df.groupby(["Locality", "Type"])[
        "Terrace Surface"
    ].transform(lambda x: x.fillna(x.median()))
    df["State of Building"] = df.groupby(["Locality", "Type", "Build Year"])[
        "State of Building"
    ].transform(lambda x: x.fillna(x.median()))
    df["EPC"] = df.groupby(["Province", "Type", "Build Year"])["EPC"].transform(
        lambda x: x.fillna(x.median())
    )
    df["Consumption Per m2"] = df.groupby(["Province", "Type", "Build Year"])[
        "Consumption Per m2"
    ].transform(lambda x: x.fillna(x.median()))
    df.fillna(df.select_dtypes(exclude="object").mean(), inplace=True)
    return df
